    ...
    for box in boxes:
        ...
        if label >= 0:
            ...
            cv2.putText(image, label_str + ' ' + str(box.get_score()),
                        (box.xmin, box.ymin - 13), cv2.FONT_HERSHEY_SIMPLEX,
...
def _main_(args):
    ...
    for i in range(len(yolos)):
        ...
        boxes += decode_netout(yolos[i][0], anchors[i], obj_thresh, nms_thresh,
                               net_h, net_w)
    ...
    cv2.imwrite(image_path[:-4] + '_detected' + image_path[-4:],
                (image).astype('uint8'))
