...
def helper():
    """
    helps to create reference .h5 file, no longer needed
    """
...
image, image_w, image_h = load_image_pixels(img_name, (input_w, input_h))
# make prediction
...
# not showing the dialog window
matplotlib.use("Agg")
# draw what we found
draw_boxes(img_name, v_boxes, v_labels, v_scores)
saved = os.listdir("Saved")
# summarize what we found
for i in range(len(v_boxes)):
    if (v_labels[i] + ".mp3") in saved:
        pass
    else:
